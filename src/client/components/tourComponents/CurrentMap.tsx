...
function CurrentMap(props: MapProps): JSX.Element {
  const { passCoords, currentWP, waypoints } = props;
  ...
  const [currentMarkerLong, setCurrentMarkerLong] = useState(currentWP.long);
  const [currentMarkerLat, setCurrentMarkerLat] = useState(currentWP.lat);
  ...
  useEffect(() => {
    setCurrentMarkerLong(currentWP.long);
    setCurrentMarkerLat(currentWP.lat);
  }, [currentWP]);

  useEffect(() => {
    if (currentMarkerLat && currentMarkerLong) {
      map.current?.flyTo({
        center: [currentMarkerLong, currentMarkerLat],
        zoom: 12,
      });
    }
  ...
  function showMarkers(points, color) {
